...
cat("average in survey is", round(mean(ages$age),1))
...
plot(ages$age, prop.act, ylim = c(0, 0.4), xlim = c(-10, 90), 
...
axis(side=1, lwd.ticks = 2, cex.axis = 1.5, at = c(0, 20, 40, 60, 80), las=1) 
axis(side=3, lwd.ticks = 2, cex.axis = 1.5, at = c(0, 20, 40, 60, 80), labels = F)
axis(side = 2, lwd.ticks = 2, cex.axis = 1.5, las = 1,
    labels = c("0.0", "0.10", "0.2", "0.3", "0.4"), at = c(0, 0.1, 0.2, 0.3, 0.4))
axis(side=4, lwd.ticks = 2, cex.axis = 1, at = c(0, 0.10, 0.20, 0.30, 0.40), labels = F)
box(lwd = 2)

curve(exp(coef(f.act)[1]) * exp(coef(f.act)[2]*x), from = -5, to = 90, 
      ylim = c(0, 0.5), add = TRUE, lwd = 2, lty = 1)

curve(exp(coef(f.act)[1]) * exp(coef(f.act)[2]*x) *exp(coef(f.act)[11]), 
      from = -5, to = 90, add = TRUE, lwd = 2, lty = 2)

legend(90, 0.42, legend = c("short shower", "long shower"), seg.len = 2.0,
...
plot(ages$age, prop.act, log = "y", ylim = c(0.001, 2.0), xlim = c(-010, 90), 
     xlab = expression(paste("Age")), cex.lab = 1.5,
     ylab = NA,
     xaxt = "n", yaxt = "n",
     pch = 22, lwd = 1.5, bg = "red", col = "black", cex = 2)
...
curve(exp(coef(f.act)[1]) * exp(coef(f.act)[2]*x), from = -5, to = 85, 
      ylim = c(0, 0.5), add = TRUE, lwd = 2, lty = 1)

curve(exp(coef(f.act)[1]) * exp(coef(f.act)[2]*x) *exp(coef(f.act)[11]), 
      from = -5, to = 90, add = TRUE, lwd = 2, lty = 2)

legend(95, 2.5, legend = c("short shower", "long shower"), seg.len = 2.0,
