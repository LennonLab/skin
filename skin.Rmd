...
# Need volume of solution for each sample (mL)

# Need volume of subsample that went onto filer (mL)

# Inside radius of filter tower (cm)
tow.rad <- 0.75

# Area of filter
filter.area <- pi * (tow.rad)^2

# Dimensions of field (cm)
field.width <- 0.00445
field.height <- 0.00335
field.cm2 <- field.width * field.height
fields.filter <- filter.area / field.cm2

# multiply counts by field per filter = cells per filter
# divide cells per filter by volume of sample filtered (uL) and multipl by 1000 to get mL
```

### C. Calculate percentages of each metabolic class
```{r}
...
### E. Make stacked bar chart for each subject and differen habitats
```{r}
