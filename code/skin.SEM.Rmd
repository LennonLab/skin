...
plot(ages.2$age,log(ages.2$total), ylim = c(8, 18), cex = 2, pch = 22, col = "black", bg = "grey")
points(ages.2$age, log(ages.2$active), cex = 2, pch = 22, col = "black", bg = "red")
...
# What happens when you increase frequency of showers?
# While noting that show.freq only influences active
f3.tot <- exp(coef(f.tot)[1])*exp(coef(f.tot)["show.f3"])

cat("More frequent showers reduce total bacteria by", round((ave.tot - f3.tot)/ave.tot*100, 1), "%")

# More frequent showers increases proportion of active cells 
f3.effect <-rbind(ave.status, exp(coef.matrix[,1])*exp(coef.matrix[,"show.f3"]))
f3.diff <- f3.effect[2,] - f3.effect[1,] 
f3.effect <- rbind(f3.effect, f3.diff)
rownames(f3.effect) <- c("ave.status", "f3.effect", "diff")
f3.effect
